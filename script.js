...
async function solve() {
    ...
    try {
        const response = await fetch(`${API_URL}/solve`, {
            ...
            body: JSON.stringify({ method: currentMethod, data: data, degree: parseInt(degree) })
        });
        ...
        // DIRECT HTML INJECTION FOR THE NEW FORMATTING:
...
async function predictY() {
    ...
    try {
        const response = await fetch(`${API_URL}/predict`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ method: currentMethod, coefficients: currentCoeffs, x: xVal })
        });
...
function drawGraph(graphData, rawData) {
    ...
    let minX = Math.min(...points.map(p => p.x), ...graphData.x);
    let maxX = Math.max(...points.map(p => p.x), ...graphData.x);
